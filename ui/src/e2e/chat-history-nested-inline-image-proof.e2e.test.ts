...
const exactPrHead = "56347bbdf88020dbda839be75188932acb45f864";
...
suite.define(() => {
  it("shows a nested stored-history omission while preserving a live image", async () => {
    ...
    try {
      ...
      const omissionCard = page.locator(".chat-inline-media-omitted");
      await omissionCard.waitFor({ state: "visible", timeout: 10_000 });
      await expect.poll(() => omissionCard.count()).toBe(1);
      await expect.poll(() => omissionCard.textContent()).toContain("Image unavailable in history");

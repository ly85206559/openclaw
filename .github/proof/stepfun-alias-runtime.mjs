import assert from "node:assert/strict";
import { spawn } from "node:child_process";
import fs from "node:fs/promises";
import http from "node:http";
import os from "node:os";
import path from "node:path";

const exactHead = process.env.PR_SOURCE_HEAD;
assert.ok(exactHead, "PR_SOURCE_HEAD is required");

const scenarios = [
  { provider: "stepfun-ai", model: "step-3.7-flash" },
  { provider: "stepfun-ai-step-plan", model: "step-3.5-flash-2603" },
];

function modelRow(id) {
  return {
    id,
    name: `Proof ${id}`,
    reasoning: false,
    input: ["text"],
    cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
    contextWindow: 8192,
    maxTokens: 1024,
  };
}

async function runCli(args, env) {
  const child = spawn(process.execPath, [path.resolve("openclaw.mjs"), ...args], {
    cwd: process.cwd(),
    env,
    stdio: ["ignore", "pipe", "pipe"],
  });
  let stdout = "";
  let stderr = "";
  child.stdout.setEncoding("utf8");
  child.stderr.setEncoding("utf8");
  child.stdout.on("data", (chunk) => (stdout += chunk));
  child.stderr.on("data", (chunk) => (stderr += chunk));
  const code = await new Promise((resolve, reject) => {
    child.once("error", reject);
    child.once("close", resolve);
  });
  assert.equal(code, 0, `openclaw ${args.join(" ")} failed\n${stderr}\n${stdout}`);
  return { stdout, stderr };
}

async function proveScenario(scenario) {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), `stepfun-${scenario.provider}-`));
  const stateDir = path.join(root, "state");
  const configPath = path.join(root, "openclaw.json");
  const workspace = path.join(root, "workspace");
  await fs.mkdir(workspace, { recursive: true });
  const requests = [];
  const server = http.createServer((request, response) => {
    void (async () => {
      let raw = "";
      for await (const chunk of request) raw += String(chunk);
      const body = JSON.parse(raw);
      requests.push({
        path: new URL(request.url ?? "/", "http://127.0.0.1").pathname,
        authorization: request.headers.authorization,
        model: body.model,
        stream: body.stream,
      });
      const chunk = {
        id: `proof-${scenario.provider}`,
        object: "chat.completion.chunk",
        created: 1,
        model: scenario.model,
        choices: [{ index: 0, delta: { role: "assistant", content: "ALIAS_OK" }, finish_reason: null }],
      };
      response.writeHead(200, { "content-type": "text/event-stream" });
      response.end(
        `data: ${JSON.stringify(chunk)}\n\ndata: ${JSON.stringify({ ...chunk, choices: [{ index: 0, delta: {}, finish_reason: "stop" }] })}\n\ndata: [DONE]\n\n`,
      );
    })().catch((error) => response.destroy(error));
  });
  await new Promise((resolve, reject) => {
    server.once("error", reject);
    server.listen(0, "127.0.0.1", resolve);
  });

  try {
    const address = server.address();
    assert.ok(address && typeof address !== "string", "proof server did not bind");
    const baseUrl = `http://127.0.0.1:${address.port}/v1`;
    await fs.writeFile(
      configPath,
      `${JSON.stringify({
        gateway: { mode: "local" },
        plugins: { slots: { memory: "none" } },
        agents: {
          defaults: {
            workspace,
            skipBootstrap: true,
            skills: [],
            model: { primary: `${scenario.provider}/${scenario.model}` },
          },
        },
        models: {
          providers: {
            [scenario.provider]: {
              baseUrl,
              api: "openai-completions",
              apiKey: "proof-placeholder",
              headers: { "X-StepFun-Proof": scenario.provider },
              models: [modelRow(scenario.model)],
            },
          },
        },
      }, null, 2)}\n`,
      "utf8",
    );
    const env = {
      PATH: process.env.PATH,
      HOME: root,
      USERPROFILE: root,
      CI: "1",
      NO_COLOR: "1",
      OPENCLAW_CONFIG_PATH: configPath,
      OPENCLAW_STATE_DIR: stateDir,
      OPENCLAW_TEST_FAST: "1",
    };

    const status = await runCli(["models", "status", "--json"], env);
    const statusJson = JSON.parse(status.stdout);
    assert.equal(statusJson.defaultModel, `${scenario.provider}/${scenario.model}`);

    const list = await runCli(["models", "list", "--provider", scenario.provider, "--json"], env);
    const listJson = JSON.parse(list.stdout);
    const listedModels = listJson.models;
    assert.ok(Array.isArray(listedModels), `unexpected models list payload: ${list.stdout}`);
    assert.ok(
      listedModels.some((entry) => entry.key === `${scenario.provider}/${scenario.model}`),
      `alias model absent from models list: ${list.stdout}`,
    );

    const agent = await runCli(
      ["agent", "--local", "--agent", "main", "--message", "return alias proof", "--json"],
      env,
    );
    assert.match(agent.stdout, /ALIAS_OK/);
    assert.deepEqual(requests, [
      {
        path: "/v1/chat/completions",
        authorization: "Bearer proof-placeholder",
        model: scenario.model,
        stream: true,
      },
    ]);
    return {
      provider: scenario.provider,
      defaultModel: statusJson.defaultModel,
      listedModel: `${scenario.provider}/${scenario.model}`,
      resolvedTransport: requests[0],
      responseObserved: "ALIAS_OK",
    };
  } finally {
    server.closeAllConnections();
    await new Promise((resolve) => server.close(resolve));
    await fs.rm(root, { recursive: true, force: true });
  }
}

const results = [];
for (const scenario of scenarios) results.push(await proveScenario(scenario));
const output = { exactHead, results };
await fs.mkdir(path.dirname(process.env.PROOF_OUTPUT), { recursive: true });
await fs.writeFile(process.env.PROOF_OUTPUT, `${JSON.stringify(output, null, 2)}\n`, "utf8");
console.log(`[behavior-evidence] stepfun-alias-runtime ${JSON.stringify(output)}`);

...
// Prints timing evidence suitable for PR body / ClawSweeper re-review.
...
function assertArgvClassifier() {
  let passed = 0;
  for (const testCase of argvCases) {
    const actual = isForegroundGatewayRunInvocation(testCase.argv);
    if (actual !== testCase.expected) {
      throw new Error(
        `argv classifier failed for ${JSON.stringify(testCase.argv)}: expected ${testCase.expected}, got ${actual}`,
      );
    }
    passed += 1;
  }
  console.log(`argv classifier proof: ${passed}/${argvCases.length} cases passed`);
}
...
function proofPackagedLauncherE2e() {
  ...
  console.log("packaged launcher proof: real openclaw.mjs fixture completed 3025 ms cleanup under SIGTERM");
}

async function main() {
  assertArgvClassifier();
